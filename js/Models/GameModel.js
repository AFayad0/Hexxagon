...
var GameModel = function(){
    ...
    this.initHexagons = function(){
        ...
        for(var i = 0, row = HEXAGON_ROW_COUNT.length; i< row; i++){
            ...
            for(var j = 0; j < HEXAGON_ROW_COUNT[i]; j++){
                ...
                //available = ( j > 0 && j < HEXAGON_ROW_COUNT[i] - 1);
                if((i == 3 && j == 3) ||  (i == 4 && j == 5) || (i == 5 && j == 3) )
                    available = false;
                hexagon = new Hexagon(new Coordinates(startX + Math.sqrt(3)*j*SIZE, startY), SIZE, place , available);
    ...
    this.initPearls = function(){
        var  pos, hexagon, pearl, count;
        var hexagons = this.hexagons.slice();//копирование без ссылок

        
        for(var i = 0, len = 2; i < len; i++){
            for( var j = 0; j < this.players[i].possitions.length; j++){
                pos = this.players[i].possitions[j];
                hexagon = hexagons[pos];
    ...
    this.playerHasPieces = function(player){
        var has = false; var color;
...
*/
        ...
        for(var j = 0, countJ = this.board[i].length; j < countJ; j++){
            if(this.board[i][j] && this.board[i][j].pearl){
                ...
                color = this.board[i][j].pearl.color[0];
        ...
        console.log(player.description + " " + has);
        return has;
    };
